<template>
  <div class="cart-page">
    <v-container fluid>
      <v-row>
        <v-col cols="12" class="pb-0">
          <v-breadcrumbs
            :items="['Home', 'Your Cart']"
            class="px-0"
            style="font-size: 13px"
          >
            <template v-slot:divider>
              <v-icon color="#878484">mdi-chevron-right</v-icon>
            </template>
          </v-breadcrumbs>
        </v-col>
        <v-col cols="12" class="py-0">
          <v-card class="px-0" elevation="0">
            <v-card-title
              class="px-0 pb-0 d-flex justify-space-between align-center w-100"
              style="font-size: 35px"
            >
              Your Cart
            </v-card-title>
            <div
              class="bar-box mt-4 mr-2 position-relative"
              v-if="cartItems.length"
            >
              <svg
                class="icon-shipping-truck"
                :style="`left: calc(${
                  parseInt((calcTotalPrice / 600) * 100) <= 100
                    ? parseInt((calcTotalPrice / 600) * 100)
                    : 100
                }% - 30px)`"
                viewBox="0 0 40.55 24"
                width="30"
                :fill="
                  parseInt((calcTotalPrice / 600) * 100) < 50
                    ? '#f44336'
                    : parseInt((calcTotalPrice / 600) * 100) >= 50 &&
                      parseInt((calcTotalPrice / 600) * 100) < 100
                    ? '#ff9800'
                    : '#4CAF50'
                "
              >
                <g id="Layer_2" data-name="Layer 2">
                  <g id="Layer_1-2" data-name="Layer 1">
                    <path
                      class="truck-body"
                      d="M40.43,11a3.86,3.86,0,0,0-3.68-2.65H28a1.25,1.25,0,0,1-1.43-1.43c0-2.18,0-4.35,0-6.53,0-.31-.08-.36-.37-.36H5.11a1.18,1.18,0,0,0-1.3,1.32c0,.74,0,1.48,0,2.22,0,.21-.06.27-.26.26-.36,0-.71,0-1.07,0a1.19,1.19,0,1,0,0,2.37H7.19c.43,0,.85,0,1.27,0a1,1,0,0,1,1.07,1A1.19,1.19,0,0,1,8.24,8.48H1.35a1.83,1.83,0,0,0-.47,0A1.19,1.19,0,0,0,0,9.85a1.18,1.18,0,0,0,1.19,1h9.66c.34,0,.68,0,1,0A1.19,1.19,0,0,1,13,12.47a1.26,1.26,0,0,1-1.26.76H1.24a1.19,1.19,0,1,0,0,2.38c.76,0,1.51,0,2.26,0,.26,0,.33.07.32.32,0,1,0,2.09,0,3.13A1.18,1.18,0,0,0,5.1,20.36c.63,0,1.26,0,1.9,0,.27,0,.39.06.47.36a4.55,4.55,0,0,0,8.78-.11.29.29,0,0,1,.32-.25H28.09a.3.3,0,0,1,.34.27,4.55,4.55,0,0,0,8.8,0,.31.31,0,0,1,.35-.26c.49,0,1,0,1.47,0a1.37,1.37,0,0,0,1.5-.87V11.41C40.41,11.29,40.47,11.12,40.43,11ZM32.84,21.62A2.18,2.18,0,1,1,35,19.45,2.21,2.21,0,0,1,32.84,21.62Zm-21,0A2.18,2.18,0,1,1,14,19.45,2.2,2.2,0,0,1,11.86,21.62Z"
                    ></path>
                    <path
                      class="truck-body"
                      d="M29.27,6h5.85c.1,0,.2,0,.29,0C33.64,2.72,32,.91,28.91.26V.57c0,1.68,0,3.35,0,5C28.9,5.9,29,6,29.27,6Z"
                    ></path>
                    <path
                      class="wheel"
                      fill="white"
                      d="M11.87,17.27A2.18,2.18,0,1,0,14,19.45,2.2,2.2,0,0,0,11.87,17.27Z"
                    ></path>
                    <path
                      class="wheel"
                      fill="white"
                      d="M32.85,17.27A2.18,2.18,0,1,0,35,19.45,2.22,2.22,0,0,0,32.85,17.27Z"
                    ></path>
                  </g>
                </g>
              </svg>
              <v-progress-linear
                :color="
                  parseInt((calcTotalPrice / 600) * 100) < 50
                    ? 'red'
                    : parseInt((calcTotalPrice / 600) * 100) >= 50 &&
                      parseInt((calcTotalPrice / 600) * 100) < 100
                    ? 'orange'
                    : 'green'
                "
                height="10"
                :model-value="
                  parseInt((calcTotalPrice / 600) * 100) <= 100
                    ? parseInt((calcTotalPrice / 600) * 100)
                    : 100
                "
                striped
              >
              </v-progress-linear>
            </div>
            <v-card-text class="px-0 pt-2" v-if="!cartItems.length">
              Free Shipping for all orders over $800.00
            </v-card-text>
            <v-card-text class="px-0 pt-5 text-center" v-if="!cartItems.length">
              You cart is empty
            </v-card-text>
            <v-card-text
              class="px-0 pt-2"
              v-if="cartItems.length && (600.0 - calcTotalPrice).toFixed(2) > 0"
            >
              Only ${{ (600.0 - calcTotalPrice).toFixed(2) }} away from Free
              Shipping
            </v-card-text>
            <v-card-text
              class="px-0 pt-2"
              v-if="
                cartItems.length && (600.0 - calcTotalPrice).toFixed(2) <= 0
              "
            >
              Your order now includes Free Shipping
            </v-card-text>
            <v-card-actions
              class="px-0 justify-center"
              v-if="!cartItems.length"
            >
              <v-btn
                class="mt-2"
                style="border-radius: 30px; text-transform: none"
                width="300"
                variant="outlined"
                elevation="0"
                density="compact"
                height="50"
                @click="$router.push({ name: 'home' })"
              >
                Continue Shopping
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-col>
        <v-col cols="8" class="px-3" v-if="cartItems.length">
          <v-table class="w-100">
            <thead>
              <tr>
                <th>PRODUCT</th>
                <th class="text-center">PRICE</th>
                <th class="text-center">QUANTITY</th>
                <th class="text-center">TOTAL</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in cartItems" :key="item.id">
                <td style="width: 55%">
                  <v-row class="align-center mb-4">
                    <v-col cols="5">
                      <img :src="item.thumbnail" alt="" class="w-100" />
                    </v-col>
                    <v-col cols="7">
                      <v-card-title
                        class="px-0"
                        style="
                          white-space: pre-wrap;
                          font-size: 14px;
                          line-height: 1.2;
                        "
                      >
                        {{ item.title }} Smaple - {{ item.category }}
                      </v-card-title>
                      <v-card-text class="px-0">
                        Category: {{ item.category }}
                      </v-card-text>
                    </v-col>
                  </v-row>
                </td>
                <td class="text-center" style="width: 15%">
                  <v-card-text class="px-0 pt-0">
                    ${{
                      (
                        item.price -
                        item.price * (item.discountPercentage / 100)
                      ).toFixed(2)
                    }}
                  </v-card-text>
                </td>
                <td class="text-center" style="width: 15%">
                  <div class="d-flex justify-space-between align-center">
                    <div class="counter">
                      <v-icon
                        size="19"
                        color="#9e9e9e"
                        @click="item.quantity > 1 ? item.quantity-- : false"
                      >
                        mdi-minus
                      </v-icon>
                      <input type="text" readonly v-model="item.quantity" />
                      <v-icon
                        size="19"
                        color="#9e9e9e"
                        @click="
                          item.quantity < item.stock ? item.quantity++ : false
                        "
                      >
                        mdi-plus
                      </v-icon>
                    </div>
                  </div>
                </td>
                <td class="text-center" style="width: 15%">
                  <v-card-text class="px-0 pt-0">
                    ${{
                      (
                        (item.price -
                          item.price * (item.discountPercentage / 100)) *
                        item.quantity
                      ).toFixed(2)
                    }}
                  </v-card-text>
                </td>
                <td>
                  <v-icon size="25" @click="deleteItem(item.id)">
                    mdi-close
                  </v-icon>
                </td>
              </tr>
            </tbody>
          </v-table>
          <v-divider width="100%" color="black"></v-divider>
          <v-card-text
            class="mt-4 px-0 pt-2 d-flex align-center"
            style="gap: 10px"
            v-if="cartItems.length"
          >
            <span>
              <svg
                id="Layer_1"
                data-name="Layer 1"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 179.94 179.96"
                class="icon icon-shield-check"
                width="20"
              >
                <path
                  d="M90,0,5,42.78C13.73,105.26,38.14,154.32,90,180c51.83-25.64,76.25-74.7,85-137.18Z"
                ></path>
                <polygon
                  class="cls-1"
                  points="149.83 67.57 134.81 52.55 79.31 108.05 49.74 78.48 34.72 93.5 79.15 137.94 79.31 137.78 79.47 137.94 149.83 67.57"
                  fill="white"
                ></polygon>
              </svg>
            </span>
            <span> Secure Shopping Guarantee</span>
          </v-card-text>
          <img src="@/assets/images/cart-page-cards.webp" alt="Cards" />
        </v-col>
        <v-col cols="4" class="px-3" v-if="cartItems.length">
          <v-card elevation="0">
            <v-card-text
              class="text-black"
              style="font-size: 14px; font-weight: bold"
              >ORDER SUMMARY</v-card-text
            >
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-card-text class="d-flex align-center justify-space-between">
              <span class="text-black">Subtotal</span>
              <span class="text-black font-weight-bold" style="font-size: 15px"
                >${{ calcTotalPrice }}</span
              >
            </v-card-text>
            <v-divider width="100%" color="black"></v-divider>
            <v-card-text class="text-black">Get Shipping Estimate:</v-card-text>
            <select
              class="w-100"
              style="
                padding: 10px 15px;
                border: 1px solid #9e9e9e;
                border-radius: 30px;
                font-size: 14px;
              "
            >
              <option
                :value="country"
                v-for="country in countries"
                :key="country"
              >
                {{ country }}
              </option>
            </select>
            <div class="states d-flex justify-space-between align-center mt-4">
              <select
                style="
                  width: 53%;
                  padding: 10px 15px;
                  border: 1px solid #9e9e9e;
                  border-radius: 30px;
                  font-size: 14px;
                "
              >
                <option :value="region" v-for="region in regions" :key="region">
                  {{ region }}
                </option>
              </select>
              <input
                style="
                  width: 45%;
                  padding: 10px 15px;
                  border: 1px solid #9e9e9e;
                  border-radius: 30px;
                  font-size: 14px;
                "
                type="text"
                placeholder="Postal Code"
              />
            </div>
            <v-card-actions class="px-0">
              <v-btn
                class="calc-btn w-100 my-4"
                variant="elevated"
                elevation="0"
                color="#0a6cdc"
                density="compact"
                height="50"
              >
                Calculate Shipping
              </v-btn>
            </v-card-actions>
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-card-text class="d-flex align-center justify-space-between">
              <span class="text-black">TOTAL:</span>
              <span class="text-black font-weight-bold" style="font-size: 15px"
                >${{ calcTotalPrice }}</span
              >
            </v-card-text>
            <v-divider width="100%" color="black"></v-divider>
            <v-divider width="100%" color="black"></v-divider>
            <v-card-actions class="px-0 mt-4" style="flex-wrap: wrap">
              <v-btn
                class="checkout-btn w-100"
                variant="elevated"
                elevation="0"
                color="#0a6cdc"
                density="compact"
                height="50"
                @click="toCheckout"
              >
                Proceed to Checkout
              </v-btn>
              <v-btn
                class="checkout-btn w-100 mt-2"
                variant="outlined"
                elevation="0"
                density="compact"
                height="50"
                @click="$router.push({ name: 'home' })"
              >
                Continue Shopping
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
  </div>
</template>

<script>
import { CartModule } from "@/stores/cart";
import { mapState, mapActions } from "pinia";

export default {
  data: () => ({
    countries: ["United States", "United Kingdom", "Spain"],
    regions: ["Albama"],
  }),
  computed: {
    ...mapState(CartModule, ["cartItems"]),
    calcTotalPrice() {
      let total = 0;
      this.cartItems.forEach((item) => {
        total +=
          (item.price - item.price * (item.discountPercentage / 100)) *
          item.quantity;
      });
      return total.toFixed(2);
    },
  },
  methods: {
    ...mapActions(CartModule, [
      "getCartItems",
      "deleteItem",
      "setToLocalstorage",
    ]),
    toCheckout() {
      this.setToLocalstorage();
      this.$router.push({ name: "check-out" });
    },
  },
};
</script>

<style lang="scss" scoped>
.cart-page {
  table {
    th {
      font-size: 12px;
      font-weight: bold;
    }
  }
  .v-card-title {
    font-size: 17px;
    font-weight: bold;
  }
  .v-card-text {
    color: #6f6f6f;
    padding-left: 0px !important;
    padding-right: 0px !important;
  }
  .calc-btn,
  .checkout-btn {
    text-transform: none;
  }
  .calc-btn {
    border-radius: 30px;
  }
  .checkout-btn {
    border-radius: 0;
  }
  .shopping-btn {
    border-color: #ddd;
  }
  .icon-shipping-truck {
    position: absolute;
    bottom: 50%;
    z-index: 1;
    transition: 0.3s;
  }
  .counter {
    border: 1px solid #9e9e9e;
    width: fit-content;
    padding: 10px 5px;
    border-radius: 30px;
    i {
      cursor: pointer;
    }
    input {
      border: none;
      outline: none;
      width: 40px;
      text-align: center;
      color: inherit;
    }
  }
  img {
    width: 270px;
  }
}
</style>
