<template>
  <div class="checkout-page">
    <v-container fluid class="ma-0 pa-0">
      <v-row>
        <v-col cols="7">
          ...
          >
            <v-card-title class="font-weight-bold py-0 px-0">
              new-ella-demo
            </v-card-title>
            <v-breadcrumbs
            ...
            <v-card-actions class="justify-space-between align-center px-0">
              <v-btn
                color="#592ff4"
                variant="elevated"
                class="flex-grow-1"
            ...
            <v-card-title class="px-0" style="font-size: 15px">
              Shipping Address
            </v-card-title>
            <select class="w-100">
              <option
            ...
            <div
              class="user-name d-flex justify-space-between align-center w-100 mt-4"
              style="gap: 15px"
            >
              <input
                type="text"
                ...
                class="w-100"
              />
              <input type="text" placeholder="Last name" class="w-100" />
            </div>
            <input type="text" placeholder="Address" class="w-100 mt-4" />
            <input
              type="text"
              placeholder="Apartmen, suite, etc. (optional)"
              class="w-100 mt-4"
            />
            <div
              class="city d-flex justify-space-between align-center w-100 mt-4"
              style="gap: 15px"
            >
              <input type="text" placeholder="City" class="w-100" />
              <input type="text" placeholder="Governorate" class="w-100" />
            ...
            <v-divider length="100%" color="black"></v-divider>
            <v-card-text style="font-size: 12px; color: #6f6f6f">
        ...
        <v-col cols="5">
          ...
          >
            <v-card
              ...
              color="transparent"
              width="100%"
              class="d-flex justify-space-between align-center mb-3 pt-2"
            >
              <v-badge :content="item.quantity" color="#808080">
                <img :src="item.thumbnail" alt="" width="50" height="50" />
              </v-badge>
              <div class="title-box flex-grow-1">
                <v-card-title class="item-title py-0">{{
                  item.title
                }}</v-card-title>
                <v-card-text class="category">{{ item.category }}</v-card-text>
              </div>
              <v-card-text class="item-price flex-grow-0">
                ${{
                  (
                    (item.price -
    ...
    <OrderSuccess :popup="dialog" v-if="dialog" @close-popup="dialog = false" />
  </div>
</template>
